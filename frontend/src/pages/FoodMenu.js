...
function FoodMenu() {
  ...
  useEffect(() => {
    // Load food menu categories from the JSON file in the public folder
    axios
      .get("https://medina-restaurant.ir/api/items/?format=json")
      .then((response) => {
        const foodCategories = response.data.categories.filter(
          (category) => category.id <= 3
        ); // Filter for food categories
        setCategories(foodCategories);
        fetchCards(foodCategories[0].id);
      })
      .catch((error) => {
  ...
  const fetchCards = (categoryId) => {
    // Load food menu items from the JSON file in the public folder
    axios
      .get("https://medina-restaurant.ir/api/items/?format=json")
      .then((response) => {
