...
function DrinkMenu() {
  ...
  useEffect(() => {
    // Load drink menu categories from the JSON file in the public folder
    axios
      .get("https://medina-restaurant.ir/api/items/?format=json")
      .then((response) => {
        const drinkCategories = response.data.categories.filter(
          (category) => category.id >= 4 && category.id <= 7
        ); // Filter for drink categories
        setCategories(drinkCategories);
        fetchCards(drinkCategories[0].id);
      })
      .catch((error) => {
  ...
  const fetchCards = (categoryId) => {
    // Load drink menu items from the JSON file in the public folder
    axios
      .get("https://medina-restaurant.ir/api/items/?format=json")
      .then((response) => {
      ...
      <header className="flex items-center justify-center rounded-3xl p-2 shadow-lg gap-[1em] italiana-regular">
        ...
        <nav className="categories-nav" ref={navRef}>
          {categories.length > 0 && (
            ...
            >
              <div className="category-icon selected">
                <img src="/icons/beverage.png" alt="Beverage icon" />
              </div>
              <span className="text-xs">نوشیدنی</span>
            </div>
          )}
        </nav>
